...
# Pull flatpak data from the netshare backup back onto this host (copy-only).
# Layout mirrors root so you can drop home/ into $HOME and system bits into /var and /etc.
# 
# Env toggles:
#   REMOTE_BASE      smb rclone remote/path (default: smb0:data/netshare/dada/flatpak)
...
REMOTE_BASE_DEFAULT="smb0:data/netshare/dada/flatpak"
