...
load_local_env() {
    ...
    echo "ERROR: No certificate config found. Copy config/certs.example.env to config/certs.local.env or provide YGG_CERTS_ENV_FILE." >&2
    exit 1
}

load_local_env

CERT_PRIMARY_DOMAIN="${CERT_PRIMARY_DOMAIN:?CERT_PRIMARY_DOMAIN is required}"
LEGO_DOMAINS="${LEGO_DOMAINS:?LEGO_DOMAINS is required}"
LEGO_ACCOUNT_EMAIL="${LEGO_ACCOUNT_EMAIL:?LEGO_ACCOUNT_EMAIL is required}"
LEGO_DNS_PROVIDER="${LEGO_DNS_PROVIDER:-porkbun}"
LEGO_KEY_TYPE="${LEGO_KEY_TYPE:-rsa2048}"
LEGO_STORAGE_PATH="${LEGO_STORAGE_PATH:-/etc/letsencrypt/${CERT_PRIMARY_DOMAIN}}"
LEGO_INSTALL_DIR="${LEGO_INSTALL_DIR:-${LEGO_STORAGE_PATH}/lego}"
LEGO_EXEC_NAME="${LEGO_EXEC_NAME:-lego}"
...
if [[ "$LEGO_DNS_PROVIDER" == "porkbun" ]]; then
    ...
    : "${PORKBUN_API_KEY:?PORKBUN_API_KEY is required for porkbun}"
fi

read -r -a DOMAINS_TO_CERTIFY <<< "$LEGO_DOMAINS"
if [[ "${#DOMAINS_TO_CERTIFY[@]}" -eq 0 ]]; then
    echo "ERROR: LEGO_DOMAINS did not yield any domains." >&2
    exit 1
fi

export LEGO_DEBUG_CLIENT_VERBOSE_ERROR="${LEGO_DEBUG_CLIENT_VERBOSE_ERROR:-true}"
export LEGO_DEBUG_ACME_HTTP_CLIENT="${LEGO_DEBUG_ACME_HTTP_CLIENT:-true}"
...
PRIMARY_DOMAIN_CERT_NAME="${CERT_PRIMARY_DOMAIN}"
